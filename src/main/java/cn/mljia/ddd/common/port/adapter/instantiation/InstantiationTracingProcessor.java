...
public class InstantiationTracingProcessor implements ApplicationListener<ContextRefreshedEvent> {
	...
	private static final Map<String, Class<?>> domainEvents = new HashMap<String, Class<?>>();
	...
	public void loaderDomainEvents(String packageName, Class<?> event) throws ClassNotFoundException {
		...
		for (Class<?> clas : returnClassList) {
			domainEvents.put(clas.getName(), clas);
		}
	}

	public void postThreadPoolExecutor() {

		for (Map.Entry<String, Class<?>> event : domainEvents.entrySet()) {
			new Thread(new DistributeEventTracker(this.getThreadPoolTaskExecutor(), event.getKey(),
					this.getNotificationApplicationService(), this.getLock(),
					this.getCommonConfiguration().getDomainName(), this.messageProducer(event.getKey()))).start();
		}
	...
	private MessageProducer messageProducer(String exchangeName) {
		...
		Exchange exchange = Exchange
				.fanOutInstance(ConnectionSettings.instance(this.getRabbitmqConfiguration().getAddress(),
						this.getRabbitmqConfiguration().getVirtualHost(), this.getRabbitmqConfiguration().getUsername(),
