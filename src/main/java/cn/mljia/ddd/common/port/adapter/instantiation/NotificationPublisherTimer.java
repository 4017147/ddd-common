...
public class NotificationPublisherTimer extends Thread {
	...
	private static Logger logger = LoggerFactory
			.getLogger(NotificationPublisherTimer.class);
	...
	public NotificationPublisherTimer(String typeName,
	...
	public void run() {
		...
		if (logger.isDebugEnabled()) {
			...
			logger.debug(
					"Notification Message Tracker Name======>{},currentTime:====>{}",
					this.getTypeName(), dateFormat.format(new Date()));
		}
		...
		try {
			...
			try {
				...
				if (tryLock) {
					this.getNotificationApplicationService()
							.publishNotifications(this.getMessageProducer(),
									this.getTypeName());

				} else {
					if (logger.isDebugEnabled()) {
						logger.debug(
							"tryLock not get lock thread-name:=====>{},lockName:=======>{},tryLock-status:=======>{}",
							Thread.currentThread().getName(), lockName, tryLock);
					}
				}
			} catch (Exception e) {
				logger.error("current Tracker Name======>{}",
						this.getTypeName());
				...
				if (tryLock) {
					if (this.getLock().unLock(this.getLockName())) {
						if (logger.isDebugEnabled()) {
							logger.debug(
									"thread-name:=====>{},lockName:=======>{},unLock-status:=======>{}",
						...
						if (logger.isDebugEnabled()) {
							logger.debug(
									"finally tryLock not get lock thread-name:=====>{},lockName:=======>{},unLock-status:=======>{}",
									Thread.currentThread().getName(), lockName,
									"fail");
						}
					}
			...
			e.printStackTrace();
			System.out
					.println("Problem publishing notifications from ApplicationServiceLifeCycle.");
		}
	}
	...
	private void setNotificationApplicationService(
			NotificationApplicationService notificationApplicationService) {
